...
# Maximum SNPs allowed for downsampling
MAX_SNPS_LIMIT = 50000


def clean_sample(s):
    """Strip suffixes like .bam, .sorted.bam, etc."""
    return s.split(".")[0]
...
def pca_mds(species: str, req: dict):
    ...
    return {
        ...
        "pca": {"coords": coords_pca.tolist(), "explained": explained},
        "mds": {"coords": coords_mds.tolist()},
...
def heatmap(species: str, req: dict):
    ...
    # Cap by backend limit
    if max_snps > MAX_SNPS_LIMIT:
        print(f"[HEATMAP] Requested {max_snps} SNPs, cap is {MAX_SNPS_LIMIT}")
    ...
    return {
        ...
        "n_variants": int(variants_used),      # FIXED
        "max_snps_used": int(variants_used),   # FIXED
        "n_samples": len(reordered_samples),
    }
